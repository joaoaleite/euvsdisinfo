...
language_weights = {
    language: len(df[df["article_language"] == language]) / len(df) for language in df["article_language"].unique()
}
...
for target_language in languages:
    for seed in range(10):
        ...
        random.seed(seed)
        np.random.seed(seed)
        df_lang = df[df["article_language"] == target_language]
        df_others = df[df["article_language"] != target_language]
        train_df, test_df = train_test_split(df_others, test_size=0.3, random_state=seed)
        test_df = pd.concat([test_df, df_lang])

        num_neg, num_pos = len(df[df["label"] == 0]), len(df[df["label"] == 1])
        print(target_language, len(train_df), len(test_df), num_pos, num_neg)
...
print(results)
results.to_csv("results_llo.csv", index=False)
